...
def TeamRanks(date):
	data = urllib.urlopen("http://www.koreabaseball.com/TeamRank/TeamRank.aspx?searchDate=" + str(date))
	soup = BeautifulSoup(data)
	soup = BeautifulSoup(soup.prettify(formatter=None))
	scores = soup.find_all("table", {"class":"tData"})
	teamRanks = []
...
def TeamRecords():
	data = urllib.urlopen("http://www.koreabaseball.com/Record/TeamRecord.aspx")
	soup = BeautifulSoup(data)
...
def InsertTeams(teams):
	...
	for sorted_team in sorted_teams:
		...
		if not cur.fetchone()[0]:
			print "InsertTeams. sorted_team : ", sorted_team
...
def InsertTeamRecords():
	...
	for teamRecord in teamRecords:
		...
		if not cur.fetchone()[0]:
			print "InsertTeamRecords. teamRecord : ", teamRecord
