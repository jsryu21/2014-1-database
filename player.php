...
<head>
	...
	<style type="text/css" style="opacity:0.4;">
	#opa{
		position: relative;
...
if($result->num_rows>0){
	if($result->num_rows>1){
		...
		while($row=$result->fetch_assoc()){
			...
			echo "<tr><td>$i</td><td><a href=\"#{$i}\">".$row['name']."</a></td><td>".$team['name']."</td><td>".$row['no']."</td></tr>";
			$i=$i+1;
		}
		echo "</table>";
		echo "</div>";
	...
	echo "해당 이름의 선수 정보가 없습니다. 입력하신 선수 이름 : {$searchKey}<br />";
}
mysqli_close($con);
?>
