...
foreach($names as $name){
	$nameQuery.=" OR name='$name'";
	$nameStr.=",$name";
}
$name=substr($nameQuery,4);
$names=substr($nameStr,1);

$result=mysql_query("SELECT player_id FROM player WHERE $name");
$pID=array();
...
echo "<div id=\"top\">";
echo "<a href=\"#pitcherRank\">투수 순위</a><br />";
echo "<a href=\"#batterRank\">타자 순위</a><br />";
echo "</div>";
echo "<br />";
echo "<div id=\"pitcherRank\">";
...
while($row=mysql_fetch_assoc($result)){
	...
	if($colName=$engToKor[$row['COLUMN_NAME']]){
		echo "<td><a href=./top.php?rank={$_GET['rank']}&pStat={$row['COLUMN_NAME']}&bStat={$_GET['bStat']}&name={$names}>{$colName}</a></td>";
	}else{
		echo "<td><a href=./top.php?rank={$_GET['rank']}&pStat={$row['COLUMN_NAME']}&bStat={$_GET['bStat']}&name={$names}>{$row['COLUMN_NAME']}</a></td>";
	}
}
echo "</tr>";
...
if(in_array($pStat,$specialCase)){
	if($pStat=='era'||$pStat=='aobp'||$pStat=='aslg'||$key=='aavg'){
		if(!$result=mysql_query("SELECT * FROM (SELECT (SELECT COUNT(*)+1 FROM t_pitcher WHERE ip>=t_g AND $pStat<t.$pStat) AS rank,t.* FROM (SELECT * FROM t_pitcher WHERE ip>=t_g) AS t ORDER BY rank) AS rt WHERE rank<=$n OR $name")){
			die('query error');
		}
	}else{
		if(!$result=mysql_query("SELECT * FROM (SELECT (SELECT COUNT(*)+1 FROM t_pitcher WHERE ip>=t_g AND $pStat>t.$pStat) AS rank,t.* FROM (SELECT * FROM t_pitcher WHERE ip>=t_g) AS t ORDER BY rank) AS rt WHERE rank<=$n OR $name")){
	...
	if(!$result=mysql_query("SELECT * FROM (SELECT (SELECT COUNT(*)+1 FROM t_pitcher WHERE $pStat>t.$pStat) AS rank,t.* FROM t_pitcher AS t ORDER BY rank) AS rt WHERE rank<=$n OR $name")){
		die("querry error");
	}
}
...
while($row=mysql_fetch_assoc($result)){
	...
	if($colName=$engToKor[$row['COLUMN_NAME']]){
		echo "<td><a href=./top.php?rank={$_GET['rank']}&pStat={$_GET['pStat']}&bStat={$row['COLUMN_NAME']}&name={$names}>{$colName}</a></td>";
	}else{
		echo "<td><a href=./top.php?rank={$_GET['rank']}&pStat={$_GET['pStat']}&bStat={$row['COLUMN_NAME']}&name={$names}>{$row['COLUMN_NAME']}</a></td>";
	}
}
echo "</tr>";

if(in_array($bStat,$specialCase)){
	if(!$result=mysql_query("SELECT * FROM (SELECT (SELECT COUNT(*)+1 FROM t_batter WHERE pa>=t_g*3.1 AND $bStat>t.$bStat) AS rank,t.* FROM (SELECT * FROM t_batter WHERE pa>=t_g*3.1) AS t ORDER BY rank) AS rt WHERE rank<=$n OR $name")){
		die('query error');
	}
}else{
	if(!$result=mysql_query("SELECT * FROM (SELECT (SELECT COUNT(*)+1 FROM t_batter WHERE $bStat>t.$bStat) AS rank,t.* FROM t_batter AS t ORDER BY rank) AS rt WHERE rank<=$n OR $name")){
		die("querry error");
	}
}
